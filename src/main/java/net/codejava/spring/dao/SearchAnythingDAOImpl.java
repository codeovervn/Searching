...
public class SearchAnythingDAOImpl implements SearchAnythingDAO {
	...
	public SearchAnythingDAOImpl(DataSource dataSource) {
	...
	public void saveOrUpdate(SearchAnything searchAnything) {
		...
		String searchKey = searchAnything.getSearchContents().toUpperCase();
		...
		if (searchAnything.getId() > 0) {
			...
			jdbcTemplate.update(sql, searchKey, searchAnything.getSearchContents(), resultFmDt, resultFmDt, searchAnything.getId());
		} else {
			// insert
			...
			jdbcTemplate.update(sql, null, searchKey, searchAnything.getSearchContents(), 1, resultFmDt, resultFmDt, 0);
		}
	}
	...
	public SearchAnything get(int searchAnythingId) {
		...
		return jdbcTemplate.query(sql, new ResultSetExtractor<SearchAnything>() {

			@Override
			public SearchAnything extractData(ResultSet rs) throws SQLException, DataAccessException {
				if (rs.next()) {
					SearchAnything searchAnything = new SearchAnything();
					searchAnything.setId(rs.getInt("ID"));
					searchAnything.setSearchKey(rs.getString("SEARCH_KEY"));
